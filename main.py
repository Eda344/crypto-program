...
def main(page: ft.Page):
    ...
    def decrypt_click(e):
        try:
            ...
            result_output.value = "Warning! Wrong key"
        page.update()
    ...
    def copy_click(e):
        if result_output.value and not result_output.value.startswith("Warning") and not result_output.value.startswith("first"):
            try:
                page.set_clipboard(str(result_output.value))
                snack_bar.open = True
            except Exception:
                result_output.value = "copy error"
            page.update()

    page.add(
        ft.Row([
            key_input,
            ft.ElevatedButton("Key", bgcolor="greenAccent", color="black", on_click=generate_key),
        ]),
        message_input,
        ft.Row([
        ...
        ft.Row([
            result_output,
            ft.ElevatedButton("Copy", bgcolor="greenAccent", color="black", on_click=copy_click),
        ]),
    )
